// Easing values sourced from Open Props (https://open-props.style)
// Embedded directly so the observer can resolve easing names to CSS values
// without a runtime dependency on Open Props JS.
...
/** Default config applied when data attributes are missing */
export const DEFAULTS: AnimateConfig = {
  type: 'fade',
  direction: 'up',
  duration: 700,
  delay: 0,
...
/** Named presets mapped to their 0–1 numeric equivalents */
export const INTENSITY_PRESETS: Record<IntensityPreset, number> = {
  subtle: 0.05,
  normal: 0.15,
  strong: 0.3,
};

/** Resolve an AnimateIntensity value to a normalised 0–1 number */
export function resolveIntensity(value: AnimateIntensity): number {
...
// ---------------------------------------------------------------------------
// Easing map — CSS-ready values.
// cubic-bezier as "cubic-bezier(…)" strings, spring/bounce as linear() strings.
// "spring" is aliased to "spring-3" (Open Props --ease-spring-3).
// ---------------------------------------------------------------------------

export const EASING_MAP: Record<AnimateEasing, string> = {
  // -- ease (general) -------------------------------------------------------
  ...
  // -- "spring" alias → spring-3 (replaces Motion's physics spring) ---------
  spring:
    'linear(0, 0.009, 0.035 2.1%, 0.141 4.4%, 0.723 12.9%, 0.938 16.7%, 1.017, 1.077, 1.121, 1.149 24.3%, 1.159, 1.163, 1.161, 1.154 29.9%, 1.129 32.8%, 1.051 39.6%, 1.017 43.1%, 0.991, 0.977 51%, 0.974 53.8%, 0.975 57.1%, 0.997 69.8%, 1.003 76.9%, 1)',

  // -- bounce (linear() from Open Props) ------------------------------------
  'bounce-1':
...
/** Set of all valid easing names */
export const VALID_EASINGS = new Set<string>(Object.keys(EASING_MAP));
